      ...
      <div className="w-full max-w-lg rounded-3xl bg-white p-10 text-center shadow">
        ...
        {state === "paid" && (
          <>
            ...
            <h1 className="mb-3 text-2xl font-bold text-[#082f3a]">Solde regle, merci !</h1>
            <p className="text-[#082f3a]">
              Votre solde{villaName ? ` pour ${villaName}` : ""} a bien ete encaisse. Votre reservation est desormais entierement reglee.
            </p>
            <p className="mt-4 text-sm text-[#8a755d]">
              Un recapitulatif vous a ete envoye par e-mail. Nous avons hate de vous accueillir !
            </p>
...
export default function SoldeConfirmePage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f7f1e8]" />}>
      <SoldeConfirmeContent />
    </Suspense>
  );
}
