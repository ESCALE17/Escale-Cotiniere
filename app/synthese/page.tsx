...
function SynthesePageContent() {
  ...
  const nextQuery = buildBookingQuery({ ...booking, panier: booking.panier });
  return (
    <main className="min-h-screen bg-[#f7f1e8] px-8 pt-40 pb-20">
      <section className="mx-auto max-w-4xl">
        <h1 className="mb-8 text-5xl font-bold text-[#082f3a]">{t("synthese.title")}</h1>
        <div className="rounded-3xl bg-white p-8 shadow-xl">
          {pricing.needsQuote ? (
            ...
            <div className="space-y-4 text-slate-700">
              ...
              <p><strong>{t("synthese.stay")} :</strong> {booking.arrival} - {booking.departure} ({pricing.nights} {t("synthese.nightsSuffix")})</p>
              <p><strong>{t("synthese.travelers")} :</strong> {booking.adults} {t("synthese.adult")}{booking.adults > 1 ? "s" : ""}, {booking.children} {t("synthese.child")}{booking.children > 1 ? "s" : ""}, {booking.babies} {t("synthese.baby")}{booking.babies > 1 ? "s" : ""}{booking.pet === "oui" ? t("synthese.withAnimal") : ""}</p>
              <p><strong>{t("synthese.client")} :</strong> {booking.prenom} {booking.nom} - {booking.email}</p>
              <hr />
              {pricing.weeklyDiscount > 0 && (<p className="text-green-700">Remise semaine incluse : -{pricing.weeklyDiscount.toFixed(2)} EUR</p>)}
              <p className="text-2xl font-bold text-[#082f3a]"><strong>{t("synthese.total")} :</strong> {pricing.total.toFixed(2)} EUR</p>
              <p><strong>{t("synthese.depositNow")} :</strong> {pricing.deposit.toFixed(2)} EUR</p>
              <p><strong>{t("synthese.balance")} :</strong> {pricing.balance.toFixed(2)} EUR</p>
              <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900">{t("synthese.touristTaxNote", { amount: pricing.touristTax.toFixed(2) })}</p>
            </div>
          )}
          <div className="mt-8 rounded-2xl bg-[#f7f1e8] p-6 text-sm text-slate-700">{t("synthese.payNote")}</div>
          {pricing.needsQuote ? (
            <button disabled className="mt-8 block w-full rounded-full bg-[#082f3a] px-8 py-4 text-center text-white opacity-40">{t("synthese.continue")}</button>
          ) : (
            <Link href={"/paiement?" + nextQuery} className="mt-8 block w-full rounded-full bg-[#082f3a] px-8 py-4 text-center text-white">{t("synthese.continue")}</Link>
          )}
        </div>
      </section>
    </main>
